...
public class BanCommand extends CommandBase {
    ...
    @Command(name = "", desc = "Bans a player", usage = "<player> <time> <reason>")
    @Require("xenon.punish.ban")
    public void ban(@Sender CommandSender sender, Player player, String time, @OptArg @Text String reason) {
        if (player == null) {
        ...
        XenonPlugin.getInstance().getShared().getRedis().sendPacket(new PunishmentUpdatePacket(sender instanceof Player ? ((Player) sender).getUniqueId().toString() : XenonConstants.getConsoleUUID().toString(), player.getUniqueId().toString(), PunishmentType.BAN.name(), grant.getTimeCreated(), duration, reason, false, false));
    }
}
