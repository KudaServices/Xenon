...
public class UnmuteCommand extends CommandBase {
    ...
    public void ban(@Sender CommandSender sender, Player player, @OptArg @Text String reason) {
        ...
        XenonPlugin.getInstance().getShared().getRedis().sendPacket(new PunishmentUpdatePacket(sender instanceof Player ? ((Player) sender).getUniqueId().toString() : XenonConstants.getConsoleUUID().toString(), player.getUniqueId().toString(), PunishmentType.MUTE.name(), -1, -1, reason, true, false));
    }
}
